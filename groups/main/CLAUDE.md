...
Call `mcp__ollama__ollama_list_models` first, pick the smallest available model, then call `mcp__ollama__ollama_generate`. Take the response, format it for messaging, and send it. If Ollama fails, fall back to answering yourself.
...
- **Any finance-related task**: stock analysis, portfolio review, market research, fund analysis, financial planning, reading financial files — use the stock analysis agents and delegate all analysis work to Gemini
...
For finance tasks: orchestrate using the stock analysis agents (`stock-analysis-orchestrator`, `systematic-fundamental-analyst`, `independent-technical-analyst`, `portfolio-manager`, `financial-advisor-index-funds`) and pass all heavy analysis work to Gemini via `gemini_generate` with `search: true` for live market data.
Default model: `gemini-3.0-flash`. Use `gemini-2.5-pro` for deep financial analysis requiring large context.
